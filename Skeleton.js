...
export default function Skeleton () {
	...
	const children = attributes.createAttribute("children");
	...
	this.deleteBone = function (bone) {
		bones[bone] = null;
		labelDictionary[labels[bone]] = null;
		labels[bone] = null;
		parents[bone] = null;
		children[bone] = null;
		bindTransforms[bone] = null;
		localTransforms[bone] = null;
		worldTransforms[bone] = null;
		offsetTransforms[bone] = null;
		keys[bone] = null;
		attributes.deleteElement(bone);
	}
	...
	/// fix this, wrong if hierarchy isn't ordered in the array
	this.computeWorldTransforms = function (t = 0) {
		this.computeLocalTransforms(t);
		console.log("compute world transform")
		...
		// 	console.log(bone, worldM);
		// 	worldM.multiply(localM);
		// });

		/// sort bones to appear parents first
		let sortedBones = [];
		this.foreachBone(bone => {
			const parent = parents[bone];
			if(parent == root) {
		...
		for(let i = 0; i < sortedBones.length; ++i) {
			...
			sortedBones.push(...children[bone]);
